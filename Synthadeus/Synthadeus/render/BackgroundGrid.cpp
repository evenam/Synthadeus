...
void BackgroundGrid::render(Render2D* render2d, ID2D1HwndRenderTarget * renderTarget, ID2D1SolidColorBrush ** colorPalette, IDWriteTextFormat** fontPalette)
{
	...
	p1x = gridOrigin[0] + origin[0];
	p1y = gridOrigin[1] + origin[1];
	p2x = gridOrigin[0] + gridSize[0] + origin[0];
	p2y = gridOrigin[1] + gridSize[1] + origin[1];
	...
	{
		p1x = gridOrigin[0] + (float)i + origin[0]; p2x = gridOrigin[0] +    (float)i + origin[0];
		p1y = gridOrigin[1]            + origin[1]; p2y = gridOrigin[1] + gridSize[1] + origin[1];
	...
	{
		p1y = (float)j + gridOrigin[1] + origin[1]; p2y =      (float)j + gridOrigin[1]  + origin[1];
		p1x = gridOrigin[0]            + origin[0]; p2x = gridOrigin[0] + gridSize[0]    + origin[0];
		renderTarget->DrawLine(D2D1::Point2F(p1x, p1y),
			D2D1::Point2F(p2x, p2y), colorPalette[gridColor], 0.5f);
	}
}
