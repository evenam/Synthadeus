...
void BackgroundGrid::render(Render2D* render2d, ID2D1HwndRenderTarget * renderTarget, ID2D1SolidColorBrush ** colorPalette, IDWriteTextFormat** fontPalette)
{
	...
	// vertical lines
	for (float i = gridCellSize[0]; i < gridSize[0]; i += gridCellSize[0])
	{
		p1x = gridOrigin[0] +		 i + instanceOrigin[0]; p2x = gridOrigin[0] +			i + instanceOrigin[0];
	...
	// horizontal lines
	for (float j = gridCellSize[1]; j < gridSize[1]; j += gridCellSize[1])
	{
		p1y =		 j + gridOrigin[1] + instanceOrigin[1]; p2y =			  j + gridOrigin[1]  + instanceOrigin[1];
